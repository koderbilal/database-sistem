...
class Utils {
    ...
    has(id, value) {
        ...
        if (!Array.isArray(data)) throw new DatabaseError(this.errors.mustBeArray);
    ...
    splice(id, value) {
        ...
        if (index !== -1) {
          data.splice(index, 1);
          return this._set(id, data) ? this.get(id) : null;
        }
        return null;
      }
      pull(id, value) {
      ...
      update(id, predicate, updateData) {
        ...
        const userToUpdate = data.find(predicate);
        if (userToUpdate) {
          const updatedUser = Object.assign(userToUpdate, updateData);
          return this._set(id, data) ? updatedUser : null;
        } else {
          return null;
        }
      }
